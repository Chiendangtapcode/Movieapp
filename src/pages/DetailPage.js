...
function DetailPage() {
  ...
  const {data : recommentdationData} = useFetch(`/${params?.explore}/${params?.id}/recommendations`)
    ...
    <div>
      ...
      <div className='container mx-auto px-3 py-16 lg:py-0 flex flex-col lg:flex-row gap-5 lg:gap-10'>
        ...
        <img
          ...
          />
          <button onClick={()=>handlePlayVideo(data)} className='mt-3 w-full py-2 px-4 text-center rounded bg-white text-black font-bold text-lg hover:bg-gradient-to-l from-red-500 to-orange-500 hover:scale-105 transition-all'>Play Now</button>
        </div>
        <div>
          ...
          <div className='grid grid-cols-[repeat(auto-fit,96px)] gap-5'>
            {
              castData?.cast?.map((startCast,index)=>{
                return(
                  <div>
                    <div>
                      <img src={imageURL+startCast?.profile_path}
      ...
      <div>
        ...
        <HorizontalScrollCard data={recommentdationData} heading={"Recommentdation "+params?.explore} media_type={params?.explore}/>
      </div>
      {
        playVideo && (
